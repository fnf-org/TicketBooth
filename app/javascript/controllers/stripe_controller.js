...
export default class StripeController extends Controller {
  connect = function() {
    const stripeKey = $('meta[name="stripe-key"]').attr('content') || this.element.getAttribute('data-stripe-publishable-key');

    const stripe= loadStripe(stripeKey).then()

    let elements

    isnitialize(stripeKey)
