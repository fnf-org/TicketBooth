...
array.includes-or-complain() {
  array.includes "$@" || {
    ...
    if [[ ${#output[@]} -gt 10 ]]; then
      log.error "Value ${element} must be one of the supplied values."
    else
      log.error "Value ${element} must be one of the supplied values:" "${output[@:0:10]}"
    fi
...
function array.force-range() {
  local n="$1"
  is.numeric "${n}" || {
    log.error "First argument to this function must be numeric, got ${n}" >&2
    return 1
  }
  ...
  [[ "${#@}" -gt 0 ]] || {
    log.error "Please pass additional arguments to define min/max" >&2
    return 1
 }
